 ...
 *   - gathers the terrain squares and interior triangles inside the
...
import { interiorTrianglesInBox } from "../collision/worldCollision";
...
class ShadowPoolRuntime {
  ...
  update(renderer: WebGLRenderer, scene: Scene, camera: Camera): void {
    ...
    for (const state of this.states.values()) {
      state.drewLastFrame = state.mesh.visible;
    ...
    for (const caster of shadowCasters()) {
      ...
      if (!state) continue;
      // A shadow that was hidden last frame has no current depth tile:
      // draw unoccluded until the next pass refreshes it.
      if (!state.drewLastFrame) state.material.uniforms.hasDepth.value = 0;
  ...
  renderPending(renderer: WebGLRenderer): void {
    ...
    if (!anyVisible && this.pending.length === 0) return;
