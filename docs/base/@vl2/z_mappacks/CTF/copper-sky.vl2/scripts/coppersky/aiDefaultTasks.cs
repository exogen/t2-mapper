...
$AIWeightDetectRemeq = 3400;//added - Lagg...
$AIWeightDetectVehicle = 3450;//added - Lagg...
...
function AIPickupItemTask::weight(%task, %client)
{
   ...
   for (%i = 0; %i < %itemCount; %i++)
   {
      ...
      if (!%item.isHidden())
      {
         ...
         if ((%item.getDataBlock().getName() $= "RepairPack" && %player.getInventory("RepairPack") <= 0 && %damage > 0.5 && %watTask !$= "AIODeployEquipment") || (%item.isCorpse && %item.getInventory("RepairKit") > 0 && %damage > 0.3 && %player.getInventory("RepairKit") <= 0) || (%item.getDataBlock().getName() $= "RepairPatch" && %damage > 0.3) || (%item.getDataBlock().getName() $= "RepairKit" && %damage > 0.3 && %player.getInventory("RepairKit") <= 0))
         {
   ...
   if (%task.engageRemeq > 0 && isObject(%task.engageRemeq) && AIClientIsAlive(%client) && %client.engageRemeq > 0)
   {
      ...
      if (!%task.attackInitted)
      {
         ...
         if (%hasMortar)
            ...
            %client.stepMove(%task.engageRemeq.getWorldBoxCenter(), 15.0);
      }  
   }
   //else we are done
   else
...
function AIDetectVehicleTask::monitor(%task, %client)
{
   ...
   if (%task.engageVehicle > 0 && isObject(%task.engageVehicle))// && %vehicleLOS)
   {
      //check the distance
      ...
      {
         %client.stepRangeObject(%task.engageVehicle, "BasicTargeter", 80, 300);
      ...
      {
         //%client.stepRangeObject(%task.engageVehicle, "DefaultRepairBeam", 15, 40);
         %client.stepMove(%task.engageVehicle.getWorldBoxCenter(), 30.0);

         //if (%client.getStepStatus() $= "Finished")
         %client.setTargetObject(%task.engageVehicle, 80, "Destroy");
      }
   }
   else
   {
