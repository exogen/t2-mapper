...
//AIO Escort Player
//AIO Laze Object
//AIO Mortar Object - teleport true
//AIO Repair Object
//AIO Touch Object - teleport true
...
//this function had me stumped for a long time. The use of the $Objective[Q] and also the $objectTable[] to choose objectives was the problem,
//when objectives are deleted from the $Objective[Q] they were still left in the $objTable[], the clients choose from the $objTable and so the
//objective that was deleted no longer exsisted while the client was trying to choose it. Will research this further.       - Lagg... 1-27-2004
function AIChooseObjective(%client, %useThisObjectiveQ)
{
   ...
   else
   {
      ...
      //this next line I don't get, how could you ask for so many varibles and only use 1?
   ...
   if ($objTableCount > 0 && $objTable[0, objective] != %client.objective)
   {
      ...
      if (%client.objective)
      {
         ...
         //echo(%client.nameBase @ " is being unassigned from " @ %client.objective.description);
      }
      ...
      //echo(%client.nameBase @ " is being assigned to " @ %chooseObjective.description);
...
function AITouchObject::assume(%task, %client)
{
   ...
   if (!%client.needEquipment && %task.buyEquipmentSet !$= "")
   {
	 ...
	 if (%distToObject < 0 || %closestDist < 50)
	    %client.needEquipment = true;
      }
   }
   ...
   if (%task.useTeleport)
   {
      ...
      if (%closestMPB > 0 && %closestTel > 0)
      {
         ...
         if (%closestMPBDist < %closestTelDist)
         {
	       ...
	       %closestInv = getWord(%result, 0);
               %client.invToUse = %closestInv;
               error("resetting client's inventory to use");
            }
            %task.useTeleport = false;
            return;
         }
...
function AIEscortPlayer::assume(%task, %client)
{
   ...
   else
   {
      ...
      {
         //see if we could benefit from inventory
         %needArmor = AIMustUseRegularInvStation(%task.equipment, %client);
         %result = AIFindClosestInventory(%client, %needArmor);
         %closestInv = getWord(%result, 0);
         %closestDist = getWord(%result, 1);
         if (AINeedEquipment(%task.desiredEquipment, %client) && %closestInv > 0)
         {
            ...
            if (%closestDist < 50 && (%distToTarg < 0 || %distToTarg > 150))
               %client.needEquipment = true;
         }
      }
   }
...
function AIAttackObject::initFromObjective(%task, %objective, %client)
{
   ...
   {
      %task.desiredEquipment = "SensorJammerPack";
      %task.buyEquipmentSet = "MediumEnergyELF";
   }
   else if (%randNum < 0.6)
   {
      %task.desiredEquipment = "CloakingPack";
   ...
   //error("AIAttackObject::initFromObjective - " @ getTaggedString(%client.name));
}
...
function AIAttackObject::assume(%task, %client)
{
   ...
   //error("AiAttackObject - client = " @ getTaggedString(%client.name));//more debugging
}
...
function AIODefault::weight(%objective, %client, %level, %inventoryStr)
{
   ...
   if (%needEquipment)
   {
      ...
      //echo("AIODefault::weight - %weightDist = " @ %weightDist SPC %client.nameBase);
      if (%weightDist >= 32767)
      {
...
function AIOTouchObject::weight(%this, %client, %level, %minWeight, %inventoryStr)
{
   ...
   {
      case "TouchFlipFlop":
         //added a check for siege games to enhance the ai, fix navGraph bug with forcefileds - Lagg...
